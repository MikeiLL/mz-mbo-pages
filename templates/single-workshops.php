...
<?php
	...
	$show_google_maps = 'on';//!array_key_exists( 'hide_google_maps', $details ) && $options['hide_google_maps'] !== 'on';
	...
	$calendar_timestamp_start = mz_pages_workshops_string_to_calendar( $start_date, $details['time_start_hour'], $details['time_start_minutes'], $details['time_start_ampm'], $calendar_timezone );
	$calendar_timestamp_end = ( $has_end_date && $has_end_time ? mz_pages_workshops_string_to_calendar( $end_date, $details['time_end_hour'], $details['time_end_minutes'], $details['time_end_ampm'], $calendar_timezone ) : mz_pages_workshops_string_to_calendar( $start_date, $details['time_start_hour'], $details['time_start_minutes'], $details['time_start_ampm'], $calendar_timezone, 3600 ) );
	$calendar_location =
		( empty( $details['location_address'] ) && !empty( $details['location_venue'] ) ? $details['location_venue'] . ', ' : '' ) .
		( !empty( $details['location_address'] ) ? $details['location_address'] . ', ' : '' ).
		( !empty( $details['location_city'] ) ? $details['location_city'] . ', ' : '' ) .
